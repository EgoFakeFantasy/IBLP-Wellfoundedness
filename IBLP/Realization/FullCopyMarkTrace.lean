...
include D in
/-- The full-copy case computes the exact new mark trace, including its
paired terminal source. This uses the literal new column positions. -/
theorem fullCopy_markTrace (nonempty : 0 < a.length) {b : IBLP.Pattern}
    {last row copied : IBLP.Row} {minimum p r mark imageMark target : Nat} {factors : List Nat}
    ...
    (trace : FactorTrace a target mark factors) (tail : ∀ t ∈ factors, p ≤ t) :
    ∃ imageTarget, IBLP.copyEntry a.length last target = some imageTarget ∧
      copied.columns[copied.columns.idxOf imageMark - copied.step]? = some imageTarget ∧
      IBLP.markTrace b (r + (a.length - p)) imageMark =
        some (factors.map (· + (a.length - p)) ++ [imageTarget]) := by
  ...
  obtain ⟨imageTarget, targetImage, computed⟩ := D.rawCopy_trace_compute nonempty copy hlast hm hp trace tail
  ...
  subst actual
  have markTail := tail mark trace.start_mem
  have translated := IBLP.copyEntry_tail (D.valid _ _ (IBLP.getLast_rowAt hlast)) hm hp markTail
  have sameMark : imageMark = mark + (a.length - p) := Option.some.inj (markImage.symm.trans translated)
  have newTrace : IBLP.traceFrom b imageTarget imageMark =
      some (factors.map (· + (a.length - p)) ++ [imageTarget]) := sameMark.symm ▸ computed
  exact ⟨imageTarget, targetImage, newPaired,
    by simp [IBLP.markTrace, atActual, member, legal, newPaired, newTrace]⟩
